...
export default function AboutPage() {
  return (
    <div>
      <PageHeading title="About CoE-BIRD" />
      ...
      <div className="mx-auto grid max-w-[75rem] gap-8 px-5 pb-16 lg:grid-cols-[1fr_320px]">
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card>
            <h2 className="text-xl font-bold text-[#001A23]">Inception and Evolution</h2>
            <ul className="mt-5 flex flex-col gap-5 border-l-2 border-yellow-400 pl-5">
              {timeline.map((item) => (
                <li key={item.year}>
                  <span className="block text-sm font-bold text-[#001A23]">{item.year}</span>
          ...
          <Card>
            <h2 className="text-xl font-bold text-[#001A23]">Education, Research, and Impact</h2>
            <p className="mt-4 leading-relaxed text-gray-700">
        ...
        <aside className="flex flex-col gap-6">
          <div className="flex justify-center rounded-xl border border-gray-100 bg-white p-8 shadow-sm">
            <Image src="/Assets/logos/bird.png" alt="CoE BIRD Logo" width={120} height={120} />
          </div>

          <Card>
            <h2 className="text-lg font-bold text-[#001A23]">Quick Facts</h2>
            <ul className="mt-4 flex flex-col gap-2 text-sm text-gray-700">
              {[
                "Established at IIT Delhi in 2019",
                "Focus: Bio-Inspired Robotics & Drones",
                "Interdisciplinary Research Centre",
                "M.Tech. Robotics Program (Since 2023)",
                "Collaborations with Industry & National Labs",
              ].map((fact) => (
          ...
          <Card>
            <h2 className="text-lg font-bold text-[#001A23]">Collaborations</h2>
            <ul className="mt-4 flex flex-col gap-2 text-sm text-gray-700">
              {[
